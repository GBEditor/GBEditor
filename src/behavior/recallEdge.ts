...
const recallEdgeBehavior: RecallEdgeBehavior = {
  ...
  handleNodeClick({ item }) {
    ...
    this.clearHighlightState(selectedItem => selectedItem !== item);

    if (!isSelected) {
      if (graph) graph.setItemState(item, ItemState.Selected, true);
    }
  ...
  handleEdgeClick({ item }) {
    ...
    if (isHighlight) {
      if (graph) graph.setItemState(item, ItemState.HighLight, false);
    }
  },
